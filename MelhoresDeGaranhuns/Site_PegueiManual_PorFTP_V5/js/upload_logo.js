...
function enviarArquivo(indice, barra){
	...
	request.addEventListener('load', function(e) {
		...
		if(request.response.status == "success"){
            ...
            barra.querySelector(".text").innerHTML += "<input type=\"hidden\" name=\"file-" + request.response.id +"-nome\" value=\"" + request.response.name +"\">";
            barra.querySelector(".text").innerHTML += "<input type=\"hidden\" name=\"file-" + request.response.id +"-path\" value=\"" + request.response.path +"\">";
	...
	// Caminho
	request.open('post', 'upload_logo.php'); 
	request.send(data);
}